...
class BlogEntry
  ...
  attr_accessor :title, # blog title
    :content_node,      # blog content as nokogiri node
    :author,            # full author name
    :assets,            # a hash (<anchor>=><filename> of post assests
    :blogger_name,      # short name
    :original_tags,     # original tags as defined by the imported post
    :tags,              # cleaned up tags, see normalize_tag.rb
    :camel_case_slug,   # original camel case (wiki style) slug
    :slug,              # lower cased and dash separated slug
    :date,              # post date as date object
    :lace,              # the original lace URL
    :relative_url,      # the new relative url <yyyy>-<mm>-<dd>/<slug>
    :disqus_thread_id   # disqus id for post (used by comment import)
