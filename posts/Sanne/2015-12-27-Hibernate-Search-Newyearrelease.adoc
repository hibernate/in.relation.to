...
On top of that, our same small but amazing team as been working hard on http://hibernate.org/ogm/[Hibernate OGM 5], a bit of Hibernate Validator, incredible performance improvements on Hibernate ORM "classic" and is still tinkering on the internals of Hibernate Search to make an Elasticsearch backend an alternative to plain Lucene.
