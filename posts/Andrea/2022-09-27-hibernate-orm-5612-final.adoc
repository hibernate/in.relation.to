...
A new method https://docs.hibernate.org/orm/5.6/javadocs/org/hibernate/Session.html#getReference(T)[getReference(T)] has been added to `org.hibernate.Session` that receives a detached instance and returns a reference to the persistent instance with the same identity as the given
