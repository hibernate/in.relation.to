...
This article demonstrates https://vladmihalcea.com/how-to-increment-the-parent-entity-version-whenever-a-child-entity-gets-modified-with-jpa-and-hibernate/[how you can trigger a parent entity version change whenever a child entity is added, removed or even modified].
...
You can take this idea one step further and monitor to any SQL statement type (e.g. insert, update, delete), and you can even build a tool that automatically discovers N+1 statements, as demonstrated in https://vladmihalcea.com/taming-jpa-with-the-sql-statement-count-validator/[this blog post].

I also wrote an article which aims to warn the reader about https://vladmihalcea.com/the-hibernate-enable_lazy_load_no_trans-anti-pattern/[the dangers of using the `hibernate.enable_lazy_load_no_trans` configuration property].
