...
Also, maybe using local variables for `Calendar` and a local `StringBuilder` instead of the `StringBuffer` attribute might be a better choice than having to pay the price for `synchronized`.

So now we have figured out why the timestamp has drifted from the original value.
...
The same is tru when fetching the timestamp value from the database:
