...
You can find other examples in https://docs.hibernate.org/stable/validator/reference/en-US/html_single/#section-script-evaluator-factory[our documentation], most notably examples allowing to use `@ScriptAssert` and `@ParameterScriptAssert` with SpEL (Spring Expression Language) or in OSGi environments.
