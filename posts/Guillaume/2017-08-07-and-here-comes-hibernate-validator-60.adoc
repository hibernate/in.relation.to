...
If you used the old value handling infrastructure to deal with custom containers, you need to migrate them to the http://docs.hibernate.org/stable/validator/reference/en-US/html_single/#chapter-valueextraction[new value extractor infrastructure].
...
Then, if you'd like, you can test the new features (the new constraints, the new container element support...). If you have any issues with the features or http://docs.hibernate.org/stable/validator/reference/en-US/html_single/[the documentation], please report it to us.
