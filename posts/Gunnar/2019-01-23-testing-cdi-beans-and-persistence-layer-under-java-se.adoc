...
after starting the JNDI server and binding the transaction manager itself to the JNDI tree:

[source,java]
----
public class CdiJpaTest {

    private static NamingBeanImpl NAMING_SERVER;
    ...
    public static void startJndiAndBindJtaAndDataSource() throws Exception {
        NAMING_SERVER = new NamingBeanImpl();
        NAMING_SERVER.start();
...
While you could easily encapsulate all the needed infrastructure in a base class, JUnit rule or similar,
ideally this all wouldn't be needed.
...
Simply adding the dependency to such module would then be enough to get you started with testing your CDI beans and the persistence layer under Java SE.
